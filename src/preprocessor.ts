export class PreProcessor {
    ...
    autoIndent(line: string): string {
        // Handle the auto-indentation
        const regexes: Array<[string, number]> = [];
    ...
    handleNewRpgFeatures(line: string): string {
    ...
    handleNewRpgTables(line: string): string {
        ...
        if (match) {
            ...
            if (usageMatch) extra = extra.replace(usageSearch, "");
            if (usage.includes("*update")) l += "uf";
            else if (usage.includes("*output") && !usage.includes("*input")) l += "o ";
            else l += "if";
            l += " ";
            if (usage.includes("*output") && (usage.includes("*input") || usage.includes("*update"))) l += "a";
            else l += " ";
            l += " e           ";
    ...
    handleNewRpgBlocks(line: string): string {
        ...
        if (match) {
            if (match[1] === "ds") this.datastructure = false;
            else this.parameters = false;
            return "";
        }
        return this.handleNewRpgVariables(line);
    }

    handleNewRpgVariables(line: string): string {
        let prefix: string;
    ...
    getVariableType(line: string): string {
        if (!line) return "";
        line = line.trim();
        let match;
    ...
    declareVariableType(type: string, length: string, extra: string): string {
        return `${length.padStart(7)}${type.padEnd(4)}${extra.trim()}`;
    }

    declareVariable(name: string, type: string): string {
    ...
    generateLine(type: string, name: string, definition: string): string {
